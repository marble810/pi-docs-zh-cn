...
配置文件使用与 pi 内部使用的相同的命名空间键绑定 ID ，扩展作者在 `keyHint()` 和注入的 `keybindings` 管理器中也使用这些 ID。
...
| 键绑定 ID | 默认 | 描述 |
|--------|---------|-------------|
| `tui.editor.cursorUp` | `up` | 向上移动光标，浏览顶部的更早历史记录 |
| `tui.editor.cursorDown` | `down` | 向下移动光标，浏览底部的更晚历史记录 |
...
专用的历史记录操作始终会更改历史记录条目，无论光标在多行提示中的位置如何。在主编辑器聚焦时，显式历史记录绑定优先于应用程序操作，因此将 `tui.editor.historyPrevious` 绑定到 `ctrl+p` 会在该上下文中覆盖模型循环，而不会更改选择器中的 `Ctrl+P`。
...
| `tui.select.up` | `up` | 向上移动选中内容 |
| `tui.select.down` | `down` | 向下移动选中内容 |
| `tui.select.pageUp` | `pageUp` | 列表向上翻页 |
| `tui.select.pageDown` | `pageDown` | 列表向下翻页 |
| `tui.select.confirm` | `enter` | 确认选中内容 |
| `tui.select.cancel` | `escape`, `ctrl+c` | 取消选中内容 |

### TUI 全屏视口

当交互模式使用 `--tui-mode fullscreen` 时，这些操作适用于主转录滚动区域。双指触控板和 mouse-wheel 输入会滚动指针下的区域，若指针不在固定编辑器/状态/底部停靠区域上，则回退到转录区域。点击 OSC 8 超链接会在默认处理程序中打开。使用主鼠标按钮拖动可选择文本并复制到剪贴板；在转录区域顶部或底部边缘按住会 auto-scrolls 到 off-screen 内容。有关 terminal-specific 鼠标和触控板行为，请参阅 [终端 setup](terminal-setup.md)。

全屏转录绑定优先于编辑器绑定。因此，在全屏模式下，默认的未修改导航键控制转录区域，而其 `ctrl` 变体继续控制编辑器。在全屏模式之外，两种变体都控制编辑器。
...
此路由仍可通过常规操作绑定进行配置。例如，`"tui.altScreen.pageUp": "ctrl+pageUp"` 使 `pageUp` 控制编辑器，`ctrl+pageUp` 在全屏模式下控制转录。为 half-page 步骤绑定 `tui.altScreen.halfPageUp` 和 `tui.altScreen.halfPageDown`，或为 single-line 步骤绑定 `tui.altScreen.lineUp` 和 `tui.altScreen.lineDown`。设置 `"tui.altScreen.pageUp": []` 可完全禁用该转录快捷键。用户绑定会替换该操作的默认设置。
...
| `tui.altScreen.search` | `ctrl+shift+f` | 搜索渲染后的记录 |
...
| `app.clear` | `ctrl+c` | 清除编辑器(第一个) / 退出(第二个) |
| `app.exit` | `ctrl+d` | 当编辑器为空时退出() |
...
| `app.clipboard.pasteImage` | `ctrl+v` (Windows 上为`alt+v`) | 从剪贴板粘贴图像或文本 |
...
| `app.message.copy` | `ctrl+x` | 复制最后一条助手消息，或复制 `/tree` 中选中的消息 |
...
用于作用域模型选择器(中，通过`/scoped-models`)打开。
