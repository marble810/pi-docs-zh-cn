...
- **代理** - 通过企业代理或API网关路由请求
- **自定义端点** - 使用self-hosted或私有模型部署
- **OAuth/SSO** - 为企业模型提供商添加认证流程
- **自定义 API** - 为non-standard LLM API 实现流式传输
...
- [自定义流式传输API](#custom-streaming-api)
...
扩展可以注册完整的 pi-ai `Provider`，或使用旧的 provider-config 格式。当需要自定义认证、过滤、刷新或流式行为时，建议使用完整的模型提供商。Pi 在已注册的本地模型提供商之上组合 `models.json` 覆盖。
...
扩展工厂也可以是 `async`。对于动态模型发现，请在工厂中获取并注册模型，而不是使用 `session_start`。pi 会等待工厂完成后才继续启动，因此该模型提供商在交互式启动期间可供 `pi --list-models` 使用。
...
当仅提供 `baseUrl` 和/或 `headers` 时(未提供 `models`)，该提供商下的所有现有模型将保留并使用新的端点。
...
当提供 `models` 时，它会**替换**该提供商下的所有现有模型。

`apiKey` 和自定义头值使用与 `models.json` 相同的配置值语法：开头的 `!command` 执行一条命令以获取整个值，`$ENV_VAR` 和 `${ENV_VAR}` 插入环境变量，`$` 输出字面量 ``apiKey` 和自定义头值使用与 `models.json` 相同的配置值语法：开头的 `!command` 执行一条命令以获取整个值，`$ENV_VAR` 和 `${ENV_VAR}` 插入环境变量，`$` 输出字面量 ，`$!` 输出字面量 `!`。
...
注销会移除该提供商的动态模型、API 键回退、OAuth 提供商注册以及自定义流处理程序注册。被覆盖的任何 built-in 模型或提供商行为都将恢复。
...
大多数与 OpenAI 兼容的模型提供商可以使用 `openai-completions`。使用 model-level `thinkingLevelMap` 来处理 model-specific 思考级别，并使用 `compat` 处理模型提供商的独特行为。`xhigh` 和 `max` 级别是 opt-in，需要 non-null 映射条目，并且可能被不支持的空白分隔。
...
使用 `openrouter` 实现 OpenRouter 风格的 `reasoning: { effort }` 控制。使用 `together` 实现 Together 风格的 `reasoning: { enabled }` 控制；配合 `supportsReasoningEffort`，它还会发送 `reasoning_effort`。使用 `qwen-chat-template` 用于本地兼容 Qwen 的服务器，这些服务器读取 `chat_template_kwargs.enable_thinking` 并需要 `preserve_thinking`。
使用 `cacheControlFormat: "anthropic"` 用于与 OpenAI 兼容的模型提供商，这些提供商通过系统提示词、最后一个工具定义以及最后一个用户/助手文本内容上的 `cache_control` 暴露 Anthropic 风格的提示词缓存。

对于使用 `api: "anthropic-messages"` 的 Anthropic 兼容模型提供商，在模型或模型提供商的上游模型需要自适应思考时，设置 `compat.forceAdaptiveThinking: true` (`thinking.type: "adaptive"` 加上 `output_config.effort`)。内置的自适应 Claude 模型会自动设置。仅当模型提供商发出空的思考签名并期望在重放时收到 `signature: ""` 时，才设置 `compat.allowEmptySignature: true`。

> 迁移说明： Mistral 已从 `openai-completions` 迁移到 `mistral-conversations`。
> 对于原生的 Mistral 模型，使用 `mistral-conversations`。
> 如果你有意将 Mistral 兼容/自定义端点通过 `openai-completions` 路由，请根据需要显式设置 `compat` 标志。

### 认证头｜ Auth Header

如果你的模型提供商期望 `Authorization: Bearer <key>` 但不使用标准的 API，请设置 `authHeader: true`：
...
该密钥会针对每个请求进行解析。显式的请求 `Authorization` 头优先级高于生成的值。
...
对于使用 non-standard API 的模型提供商，请实现 `streamSimple`。在编写自己的实现之前，先研究现有的提供商实现：
...
当请求超出模型的上下文窗口时， pi 可以通过压缩对话并重试来自动恢复。此恢复仅在 pi 识别出失败原因为溢出时触发。

检测在最终的助手消息上运行：

- `stopReason === "error"`
- `errorMessage` 匹配 pi 已知的溢出模式之一（详见 ([`packages/ai/src/utils/overflow.ts`](https://github.com/earendil-works/pi-mono/blob/main/packages/ai/src/utils/overflow.ts))）

如果您的模型提供商返回的溢出错误消息未被 pi 识别，请从注册该提供商的同一扩展中规范化该错误。使用 `message_end` 处理程序重写助手消息，使其 `errorMessage` 以 pi 识别的短语开头。通用回退 `context_length_exceeded` 是最安全的选择。
...
`message_end` 在 pi 跟踪助手消息以进行 auto-compaction 之前运行，因此重写后的 `errorMessage` 是 pi 检查的内容。这样， pi 将：
...
- 将其限定于您的提供商 (`message.provider` 和 `ctx.model?.provider`)，以免影响其他提供商的不相关错误。
...
| `cross-provider-handoff.test.ts` | 提供商之间的上下文移交 |
